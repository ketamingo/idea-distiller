...
// Utility to render "bite-sized" content with line breaks and list items
function BiteSizedContent({ content }: { content: string }) {
  if (!content) return null;

  // Split by double newlines for paragraphs, or single for lists
  ...
  return (
    <div className="space-y-3">
      {blocks.map((block, i) => {
        ...
        const text = isList ? block.trim().substring(1).trim() : block.trim();

        if (isList) {
          return (
            <div key={i} className="flex gap-2 items-start group/item">
              <div className="w-1.5 h-1.5 rounded-full bg-indigo-500/40 mt-1.5 flex-shrink-0 group-hover/item:bg-indigo-400 transition-colors" />
...
export default function HomePage() {
  ...
  const [placeholder, setPlaceholder] = useState("I hate correcting typos...");
  ...
  useEffect(() => {
    ...
    setPlaceholder(randomIdea);
  }, [dynamicSamples.length]); // Re-run if we get more samples

  // Ensure input is empty on mount to allow placeholder ghost text to show
  useEffect(() => {
    setInput("");
  }, []);

  // Fetch a new unique sample on every visit
  useEffect(() => {
  ...
  const handleAnalyze = async (overrideInput?: string, parentId?: string, modeOverride?: "simple" | "pro" | "executive") => {
    ...
    try {
      const res = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({ input: textToAnalyze, mode: targetMode }),
      });
  ...
  }, [mode, isPro, handleAnalyze]); // Added isPro and handleAnalyze to ensure latest context is used

  const [copied, setCopied] = useState(false);
  const handleCopy = () => {
    if (!currentResult) return;
    const text = `
Idea Distiller Concept: ${currentResult.name}
---
...
Generated by Idea Distiller
  ...
  const handleRefreshField = async (field: string, modeOverride?: "simple" | "pro" | "executive") => {
    ...
    try {
      ...
      if (data.result) {
        ...
        setIdeaHistory(prev => {
          ...
          const updatedFieldContent = (freshIdea as Record<string, any>)[field];
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              >
                ...
                <div className="flex items-start justify-between relative z-10 w-full mb-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-3xl md:text-4xl font-bold text-white leading-tight break-words">
                        {currentResult.name}
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    ...
                    {mode !== 'simple' && (
                      <button
                        onClick={() => handleAnalyze(currentResult.prompt, currentResult.parentId, 'simple')}
                        disabled={loading || regeneratingAll}
                        className="p-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 rounded-lg transition-all border border-amber-500/20 flex items-center gap-2"
                        title="Simplify the entire concept"
                      >
                        <Zap className="w-4 h-4" />
                        <span className="text-xs font-medium">Simplify All</span>
                      </button>
                    )}
                    <button
                      onClick={handleCopy}
                ...
                <div className="relative z-10 border-t border-slate-800/50 pt-6">
                  <BiteSizedContent content={currentResult.description} />
                </div>
              </motion.div>
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <Card
                  ...
                  onSimplify={mode !== 'simple' ? () => handleRefreshField('audience', 'simple') : undefined}
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <Card
                  ...
                  onSimplify={mode !== 'simple' ? () => handleRefreshField('problem', 'simple') : undefined}
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <Card
                  ...
                  onSimplify={mode !== 'simple' ? () => handleRefreshField('mvp', 'simple') : undefined}
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <Card
                  ...
                  onSimplify={mode !== 'simple' ? () => handleRefreshField('not_feature', 'simple') : undefined}
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                <Card
                  ...
                  onSimplify={mode !== 'simple' ? () => handleRefreshField('next_step', 'simple') : undefined}
    ...
    <div className={clsx("p-6 bg-slate-900 border border-slate-800 rounded-2xl group relative h-full", className)}>
        ...
        <div className="flex gap-1">
          {onSimplify && (
            <button
              onClick={onSimplify}
              disabled={loading}
              className="p-1.5 text-slate-500 hover:text-amber-400 hover:bg-amber-500/10 rounded-md transition-all disabled:opacity-30"
              title="Simplify this field"
            >
              <Zap className={clsx("w-3.5 h-3.5", loading && "animate-pulse")} />
            </button>
          )}
          {onRefresh && (
            <button
      ...
      <div className="text-slate-200 leading-relaxed font-medium">
        {loading ? (
          ...
          <BiteSizedContent content={children as string} />
        )}
      </div>
    </div>
  );
